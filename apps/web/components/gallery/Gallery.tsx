...
export default function Gallery() {
  ...
  function handleCellClick(photo: GalleryPhoto) {
    // Le click synthétique qui suit un long-press → ignoré (sinon ouvre la modale).
    ...
    <div className="flex">
            ...
            <>
              ...
              <div className="hidden items-center gap-1 md:flex">
                <Button
                  ...
                  className={cn(density === "dense" && "bg-primary/8 text-primary")}
                ...
                <Button
                  variant="ghost" size="icon-sm"
                  className={cn(density === "large" && "bg-primary/8 text-primary")}
                ...
                <div className={cn("grid gap-1", GRID_CLASS[density])}>
                  {group.photos.map((photo) => {
                    ...
                    return (
                      <div
                        key={photo.id}
                        onClick={() => handleCellClick(photo)}
                        onPointerDown={(event) => handlePointerDown(event, photo.id)}
                        onPointerMove={handlePointerMove}
                        onPointerUp={handlePointerUp}
                        onPointerCancel={handlePointerUp}
                        className={cn(
                          "group relative aspect-square cursor-pointer overflow-hidden rounded-md bg-muted select-none touch-pan-y [-webkit-touch-callout:none]",
                      ...
                      >
                        ...
                        <div className={cn(
                          ...
                          isSelected ? "bg-primary/25" : "group-hover:bg-black/15",
                        )} />

                        <button
                          onClick={(event) => { event.stopPropagation(); toggleSelect(photo.id); }}
                          className={cn(
                            "absolute left-1.5 top-1.5 z-[2] flex size-5 items-center justify-center rounded-full border-2 transition-opacity",
                            isSelected
                              ? "border-primary bg-primary opacity-100"
                              : selectionMode
                                ? "border-white bg-black/40 opacity-100"
      ...
      {activePhoto && (
        <PhotoDetailModal
          ...
          onRequestDelete={() => setPendingDeleteIds([activePhoto.id])}
        />
      )}
      ...
      <AlertDialog
        ...
        onOpenChange={(open) => { if (!open && !deleting) setPendingDeleteIds(null); }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {(pendingDeleteIds?.length ?? 0) > 1
                ? `Supprimer ${pendingDeleteIds?.length} photos ?`
                : "Supprimer cette photo ?"}
            </AlertDialogTitle>
            <AlertDialogDescription>
              Cette action est définitive.{" "}
              {(pendingDeleteIds?.length ?? 0) > 1
                ? "Les photos seront retirées"
                : "La photo sera retirée"} de votre galerie et du stockage.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
