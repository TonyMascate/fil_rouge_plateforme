...
`ConfigService`, `RedisService`, `AlbumPhotoRepository`. Cas : quota, registerUpload (succès, S3
manquant, ContentLength absent), getStatus (COMPLETED, PENDING, mauvais user, introuvable),
deletePhoto, sharePhoto (nouveau/existant token), unsharePhoto, getPublicByToken, listForUser,
getColorAtlas (cache miss/hit, filtre album sans cache), listByCell (sans album → requête directe,
avec album → chemin `findPhotosByCellPage`).

### `photo/photo.controller.spec.ts`
Mock de `PhotoService`, `AwsService`, `RedisService`. Teste tous les endpoints : quota, status,
list, atlas couleur (`getColorAtlas`), photos d'une cellule (`getColorCellPhotos` valide + cellId
inconnu → 400 + transmission de l'albumId), delete, share/unshare, multipart (quota dépassé, succès, rollback S3 sur erreur
...
**`extractPalette` (module pur `photo/color.ts`) :**
Fonction pure, testée **directement** dans `photo/color.spec.ts` : conversions sRGB↔OKLab↔OKLCH
(aller-retour), classement déterministe en cellules d'atlas (neutres, teintes vives, marron/rose),
extraction de palette (image unie, deux couleurs triées par poids, déterminisme, seuil de poids).

**`photo.processor.spec.ts` (intégration de l'extraction dans `process()`) :**
On contrôle les pixels via `mockColorFork.toBuffer.mockResolvedValue(buffer)` :

| Buffer | Chemin couvert |
|--------|---------------|
| `[255,0,0, 0,255,0, 0,0,255]` | palette extraite → `palette`, `colorCells` et `dominantColor` renseignés ; cache atlas invalidé |
| (rejet) | `colorBuf = null` → `dominantColor = null`, `palette = null` |
...
**Tests d'intégration repository (Testcontainers) :** `countByColorCell` (multi-appartenance,
scope user/COMPLETED, **non-régression du produit cartésien** avec filtre album), `findByColorCellPage`
(filtre cellule, scope, pagination), et `findPhotosByCellPage` (photos d'un album dans une cellule —
**non-régression du plantage `getManyAndCount`**).
...
## 13. Phase 4 — Tests Vitest pour `apps/web` (Next.js 16 / React 19)
...
pour éviter le conflit de `.next/dev/lock` (Next.js 16 : deux instances ne peuvent pas partager
...
**`photo.repository.spec.ts`** (10 tests) :
...
- `countByColorCell` : compte une photo dans **chacune** de ses cellules (appartenance multiple)
- `countByColorCell` : ignore les photos non-COMPLETED et celles des autres utilisateurs
- `countByColorCell` : filtré par album → **non-régression du produit cartésien** (pas de × nb photos)
- `findByColorCellPage` : renvoie les photos dont `color_cells` contient la cellule, avec le total
- `findByColorCellPage` : scope utilisateur + COMPLETED
- `findByColorCellPage` : pagination

**`album-photo.repository.spec.ts`** (9 tests) :
...
- `findPhotosByCellPage` : photos de l'album dans une cellule + total (**non-régression `getManyAndCount`**)
- `findPhotosByCellPage` : exclut hors-album, hors-cellule et limite à l'utilisateur
...
**Durée CI estimée** : ~30-45s (pull image postgres:16-alpine + startup + 22 tests).
